...
export default function RecentApplications({
  ...
  let userApplications =
    userApplicationsData != null ? userApplicationsData : [];
  ...
  return (
    <>
      <Grid item xs={12} className={classes.mainGrid} container direction="row">
        <Typography
          variant="h5"
              ...
              {isLoading ? (
                ...
                userApplications.map((appData, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell className={classes.tableheadrow} align="left">
                      {statusStr[appData.status]
                        ? statusStr[appData.status]
                        : appData.status}
                    </TableCell>
                    <TableCell align="center">
                      {appData.isActive &&
                      appData?.status !== "referred" &&
                      appData?.status !== "contact_branch" ? (
                        <ButtonPrimary
                          stylebutton='{"color":"","width":"72%" }'
                        ...
                        <ButtonPrimary
                          stylebutton='{"color":"","width":"72%","padding":"0px 30px", "fontSize":"0.938rem","fontFamily":"Muli,sans-serif" }'
                          onClick={() => viewAppData(userApplicant, appData)}
                        >
                          View
                        </ButtonPrimary>
                      )}
