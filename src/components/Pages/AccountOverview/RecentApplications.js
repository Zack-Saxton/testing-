...
export default function RecentApplications({ isLoading, userApplicationsData }) {
  ...
  const presenceOfLoan = userApplications?.some((applicant) => applicant.isActive === true && presenceOfLoanStatus?.status !== "referred" && presenceOfLoanStatus?.status !== "contact_branch");
  const presenceOfLoanStatus = userApplications?.find((applicant) => applicant.isActive === true && presenceOfLoanStatus?.status !== "referred" && presenceOfLoanStatus?.status !== "contact_branch" );
  let statusStr = {
    "approved": "Approved",
