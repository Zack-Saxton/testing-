	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			...
			let body = {
				...
				zip_code: data.zip,
				password: values.newPassword,
				birth_year: data?.dob ? data.dob.getFullYear().toString() : '',
				birth_month: data?.dob ? (data.dob.getMonth() + 1).toString().padStart(2, "0"): '',
				birth_day: data?.dob ? data.dob.getDate().toString().padStart(2, "0"): '',
