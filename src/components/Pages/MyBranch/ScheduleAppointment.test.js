...
test("Check the date and time slot is showing in the UI", async () => {
  ...
  expect(datePicker).toBeTruthy();		
	const slot = dialog.querySelector(`input[name="appointmentTime"]`);
...
test("Check the the past date is disabled and saturday and sunday also disabled", async () => {
  ...
  const todayDate = (Moment(today).format("D"));
	...
	fireEvent.click(button);	
	const dialog = screen.getByTestId("dialog");
	const submitBtn = getByText("Schedule an appointment");
...
test("When select on Tuesday", async () => {
  ...
  (Moment(today).format("D"));
	...
	await act(() => {
		const dataBtn = getByText(dateToCheck).closest('button');
	fireEvent.click(dataBtn);
	const submitScheduleButton = screen.getByTestId("submit-schedule-appointment");
...
test("When select on Monday, the time slot should not be shown1", async () => {
