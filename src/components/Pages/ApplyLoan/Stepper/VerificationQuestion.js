						...
						<ButtonPrimary
							...
							onClick={async () => {
								if (check) {
									...
									let sendData = {
										ref: responseData[0]?.fullData?.data?.questions
											?.transaction_id,
										answers: {
											question_set_id:
												responseData[0]?.fullData?.data?.questions
													?.question_set_id,
											questions: [
												{
													id: responseData[0].fullData.data.questions?.question[
														"question-id"
													],
													answer: check,
												},
									...
									let nxtRes = await APICall(
										"/integration/LexisNexis/kba_disambiguate_answer_cac?test=true",
										sendData,
										"POST",
										true
									);
									...
									if (nxtRes?.data?.data?.data?.kba) {
										setQuestionSetIdMultiple(
											nxtRes?.data?.data?.data?.kba?.questions[
												"question-set-id"
											]
										);
										setTransactionIdMultiple(
											nxtRes?.data?.data?.data?.kba["transaction-status"][
												"transaction-id"
											]
										);

										nxtRes?.data?.data?.data?.kba?.questions?.question.map(
											(val, key) => {
												tempArray.push({
													key: key,
													fullData: val,
										...
										nxtRes.data.data.result === "success" &&
										!nxtRes?.data?.data?.data?.kba &&
										nxtRes?.data?.data?.data?.kba?.failed === true
									) {
										props.setLoadingFlag(false);
										props.next();
									} else {
